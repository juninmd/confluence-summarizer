...
from src.confluence_summarizer.main import app, process_refinement_job, process_space_refinement, _perform_refinement
from src.confluence_summarizer.models.domain import RefinementJob, RefinementStatus, ConfluencePage, AnalysisResult, Critique, CritiqueSeverity
...
async def test_perform_refinement_no_critiques():
    ...
    with patch("src.confluence_summarizer.services.rag.query_context", new_callable=AsyncMock) as mock_query:
        mock_query.return_value = []
        with patch("src.confluence_summarizer.agents.analyst.analyze_content", new_callable=AsyncMock) as mock_analyze:
...
async def test_perform_refinement_with_critiques():
    ...
    analysis = AnalysisResult(critiques=[Critique(description="Bad", severity=CritiqueSeverity.HIGH, suggestion="Fix")])
    ...
    with patch("src.confluence_summarizer.services.rag.query_context", new_callable=AsyncMock) as m_query, \
         patch("src.confluence_summarizer.agents.analyst.analyze_content", new_callable=AsyncMock) as m_analyze, \
         patch("src.confluence_summarizer.agents.writer.rewrite_content", new_callable=AsyncMock) as m_rewrite, \
         patch("src.confluence_summarizer.agents.reviewer.review_content", new_callable=AsyncMock) as m_review:
...
async def test_process_refinement_job_success():
    ...
    page = ConfluencePage(id="page1", title="Title", space_key="KEY", body="Text")
    with patch("src.confluence_summarizer.services.confluence.get_page", new_callable=AsyncMock) as mock_get_page:
        mock_get_page.return_value = page
        with patch("src.confluence_summarizer.main._perform_refinement", new_callable=AsyncMock) as mock_perform:
...
async def test_process_space_refinement():
    ...
    with patch("src.confluence_summarizer.services.confluence.get_pages_from_space", new_callable=AsyncMock) as mock_get_pages, \
         patch("src.confluence_summarizer.services.rag.ingest_page", new_callable=AsyncMock) as mock_ingest, \
...
async def test_publish_page_success(mock_confluence_client):
    job = RefinementJob(id="pub-job", page_id="page1", status=RefinementStatus.COMPLETED, refined_text="New text")
    save_job_sync(job)

    page = ConfluencePage(id="page1", title="Title", space_key="SPACE", body="Old text")
    with patch("src.confluence_summarizer.services.confluence.get_page", new_callable=AsyncMock) as mock_get_page, \
         patch("src.confluence_summarizer.services.confluence.update_page", new_callable=AsyncMock) as mock_update_page:
...
async def test_publish_page_failure():
    job = RefinementJob(id="fail-job", page_id="page1", status=RefinementStatus.COMPLETED, refined_text="New text")
    save_job_sync(job)
